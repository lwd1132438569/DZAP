...
def match(name,sex,addr):
        ...
        for j in range(res.__len__()):
                if set(addr_input).issubset(addr_kw(res[j][8])):
                        res[j]=[res[j][1],res[j][2],res[j][3],res[j][8],res[j][42]]
        ...
        for k in range(res_final.__len__()):
            query_insert = "update zmap_r_patient_empi_jb set empi_id =" + "'" + str(uuid.uuid1()) + "'" + "where patient_name =" + "'" + str(res_final[k][1]) + "'" + "and sex =" + "'" + str(res_final[k][2]) + "'" + "and addr =" + "'" + str(res_final[k][3]) + "'"
