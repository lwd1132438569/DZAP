...
from empi.forms import Empi_input_jb
from empi.empilib import empi_match_jb
...
def empi_input(request):
    if request.method=="POST":  #这里POST一定要大写
        ...
        res = empi_match_jb.match(name,sex,addr)


        #获取请求内容，做验证
        f = Empi_input_jb(request.POST)  #request.POST：将接收到的数据通过Form1验证
        if f.is_valid():  #验证请求的内容和Form1里面的是否验证通过。通过是True，否则False。
            return render(request,"empi/empi_input_jb.html",{'res':res})
            ...
            return render(request,"empi/empi_input_jb.html",{"error":f.errors})
    return render(request,"empi/empi_input_jb.html")
